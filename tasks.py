...
@celery.task
def auto_deploy1(token, before, after):
    project = db.get("SELECT * FROM `t_assets_project` WHERE token='{}'".format(token))
    hosts = db.query("SELECT \
                        H.* \
                    FROM \
                        `t_deploy_auto_rule` AS AR \
                    LEFT JOIN `t_assets_hostgroup` AS HG ON ( \
                        HG.id = AR.hg_id \
                        AND AR.hg_id IS NOT NULL \
                    ) \
                    RIGHT JOIN `t_assets_host` AS H ON ( \
                        ( \
                            H.group_id = AR.hg_id \
                            AND AR.hg_id IS NOT NULL \
                        ) \
    ...
    if os.path.isdir(path):
        ...
        data['msg'].append('[WARNNING]项目repo 不存在，请初始化项目')
        os.chdir('/var/autop/repo')
        return data

    # 更新项目
    try:
        subprocess.check_output("git pull", shell=True).split('\n')
        data['msg'].append('[OK]git更新成功')

        changes = subprocess.check_output("git diff --name-status {} {}".format(before, after),shell= True).split('\n')
        deleted_file    = map(lambda x:x.split()[1], filter(lambda x:p_git_deleted_file.match(x), changes))
        updated_file    = map(lambda x:x.split()[1], filter(lambda x:p_git_updated_file.match(x), changes))
